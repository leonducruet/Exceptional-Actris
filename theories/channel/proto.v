...
Section proto.
  ...
  Qed.
  Lemma iMsg_map_texist_exist {TT:tele} f (m : TT → iMsg Σ V) :
    iMsg_map f (∃.. x, m x) ≡ (∃.. x, iMsg_map f (m x))%msg.
  Proof.
    rewrite /iMsg_texist iMsg_exist_eq=>x p. iSplit.
    + iInduction TT as [|T TT] "IH"; first by iIntros "H". simpl.
      iDestruct 1 as (p') "((%tt & H) & eq)". iExists tt.
      iApply "IH". iFrame.
    + iInduction TT as [|T TT] "IH"; first by iIntros "H". simpl.
      iDestruct 1 as (tt) "H".
      iDestruct ("IH" with "H") as (p') "(H&Heq)". iFrame.
  Qed.
