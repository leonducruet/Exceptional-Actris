...
Local Definition ghost_var_def `{!ghost_varG Σ A}
    (γ : gname) (q : dfrac) (a : A) : iProp Σ :=
  own γ (to_dfrac_agree (A:=leibnizO A) q a).
...
Notation "γ ↪ dq a" := (ghost_var γ dq a)
  (at level 20, dq custom dfrac at level 1, format "γ  ↪ dq  a") : bi_scope.

Local Ltac unseal := rewrite ?ghost_var_unseal /ghost_var_def.

Section lemmas.
  Context `{!ghost_varG Σ A}.
  Implicit Types (a b : A) (q : Qp) (dq : dfrac).

  Global Instance ghost_var_timeless γ dq a : Timeless (γ ↪{dq} a).
  Proof. unseal. apply _. Qed.

  Global Instance ghost_var_fractional γ a : Fractional (λ q, γ ↪{#q} a)%I.
  Proof. intros q1 q2. unseal. rewrite -own_op -frac_agree_op //. Qed.
  Global Instance ghost_var_as_fractional γ a q :
    AsFractional (γ ↪{#q} a) (λ q, γ ↪{#q} a)%I q.
  Proof. split; [done|]. apply _. Qed.
  Global Instance ghost_var_persistent γ a : Persistent (γ ↪□ a).
  Proof. unseal. apply _. Qed.
  ...
  Lemma ghost_var_valid_2 γ a1 q1 a2 q2 :
    γ ↪{#q1} a1 -∗ γ ↪{#q2} a2 -∗ ⌜(q1 + q2 ≤ 1)%Qp ∧ a1 = a2⌝.
  Proof.
  ...
  Lemma ghost_var_agree γ a1 dq1 a2 dq2 :
    γ ↪{dq1} a1 -∗ γ ↪{dq2} a2 -∗ ⌜a1 = a2⌝.
  Proof.
    unseal.
    iIntros "Hvar1 Hvar2". iCombine "Hvar1 Hvar2" gives %[Hq Ha]%dfrac_agree_op_valid.
    done.
  Qed.

  Global Instance ghost_var_combine_gives γ a1 q1 a2 q2 :
    CombineSepGives (γ ↪{#q1} a1) (γ ↪{#q2} a2)
      ⌜(q1 + q2 ≤ 1)%Qp ∧ a1 = a2⌝.
  Proof.
  ...
  Lemma ghost_var_unpersist γ a :
    γ ↪□ a ==∗ ∃ q, γ ↪{#q} a.
  Proof. unseal. iIntros "H".
    iMod (own_updateP with "H") as ([]) "H"; first by apply dfrac_agree_unpersist.
    iDestruct "H" as ([q [->->]%pair_eq]) "γ".
    by iExists q.
  Qed.
